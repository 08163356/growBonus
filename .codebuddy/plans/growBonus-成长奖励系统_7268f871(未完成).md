...
overview: 为5岁小侄女打造的成长奖励PWA网站，通过积分激励正向行为，支持家庭成员协作录入、孩子端深交互体验（动画+兑换）、成长报告等功能。移动端优先，可爱卡通风格。
design:
  architecture:
    component: tdesign
  styleKeywords:
...
todos:
  - id: init-project
    content: 初始化monorepo工程结构：创建前端（Vue3+Vite+TailwindCSS+Pinia+PWA）和后端（Express+TS+MongoDB）完整配置框架
    status: completed
  - id: backend-models-auth
    content: 实现后端数据模型和认证体系：全部Mongoose模型定义、JWT中间件、角色守卫、登录API、seed初始化脚本
    status: completed
    dependencies:
      - init-project
  - id: backend-core-api
    content: 实现后端核心业务API：行为模板CRUD、积分录入查询汇总、奖品管理与兑换审批、成就检测解锁、点赞、报告统计聚合
    status: completed
    dependencies:
      - backend-models-auth
  - id: frontend-base-auth
    content: 搭建前端基础架构：路由配置与角色守卫、全部Pinia stores、API服务层、类型定义、公共组件（导航栏/底部栏/弹窗/加载），实现登录页
    status: completed
    dependencies:
      - init-project
  - id: frontend-child-pages
    content: 实现孩子端三个页面：首页（积分+等级角色+记录+鼓励）、成就墙（勋章网格+详情）、奖品商城（分档+兑换+Lottie动画）
    status: completed
    dependencies:
      - frontend-base-auth
  - id: frontend-admin-pages
    content: 实现大人端三个页面：积分录入（模板快速打分+鼓励语）、管理中心（模板/奖品/审批Tab管理）、成长报告（ECharts图表+周月报）
    status: completed
    dependencies:
      - frontend-base-auth
  - id: deploy-docs
    content: 完成部署配置和文档：Nginx配置、Systemctl服务、PowerShell构建脚本、README，使用[mcp:wecom-hil]通知项目完成状态
    status: completed
    dependencies:
      - backend-core-api
...
2. **MongoDB 而非 PostgreSQL**：用户服务器已安装 MongoDB，poetry 项目也使用 MongoDB + Mongoose，无需额外搭建。文档型数据库也非常适合行为记录、积分流水等半结构化数据。

3. **Lottie-web 用于关键激励动画**：JSON 动画文件体积小（10-50KB），渲染性能优于 GIF/视频，有丰富的免费卡通动画资源。仅在关键场景使用（获得积分、兑换奖品、连续打卡），不增加整体包体积。

4. **PWA 而非原生 App / 小程序**：用户有自己的服务器和域名，PWA 可直接部署，无需应用商店审核，添加到桌面后体验接近原生，开发成本最低。

5. **单入口角色自适应**：登录后根据角色自动展示对应界面（孩子端/大人端），通过路由守卫控制权限，避免维护多个独立应用。
...
erDiagram
    ...
    User {
        ...
        string avatar
        string passwordHash
        string pin "4-digit for child"
    }
    BehaviorTemplate {
        string name
        string category "life/learn/social/achievement"
        number points
        string icon
        string description
        boolean isActive
    ...
    Prize {
        ...
        string image
        number pointsCost
        string tier "small/medium/large/super"
        string type "material/virtual"
        number stock
        boolean isActive
    }
    Redemption {
...
graph TB
    subgraph Client["移动端浏览器 / PWA"]
        A[孩子端页面] --> R[Vue Router 路由守卫]
        B[大人端页面] --> R
        R --> S[Pinia Store]
        S --> API[API Service Layer - Axios]
    end

    subgraph Server["Node.js Express 服务"]
        API --> |HTTP/JSON| MW[中间件层<br/>CORS + JWT认证 + 角色守卫]
        MW --> CT[Controllers 控制器]
        CT --> SV[Services 业务服务]
...
graph LR
    ...
    subgraph Child[孩子端]
        Home["/child 首页<br/>积分+等级+角色"]
        Wall["/child/achievements 成就墙<br/>勋章+历史"]
        Shop["/child/shop 奖品商城<br/>兑换+动画"]
    end

    subgraph Admin[大人端]
        Record["/admin 积分录入<br/>模板快速打分"]
        Manage["/admin/manage 管理中心<br/>模板+奖品+审批"]
        Report["/admin/report 成长报告<br/>图表+统计"]
    end
